...
import org.apache.spark.sql.sources.v2.{DataSourceOptions, DataSourceV2, ReadSupport, ReadSupport24, WriteSupport}
...
object DataSourceV2Relation {
  private implicit class SourceHelpers(source: DataSourceV2) {
    def asReadSupport: ReadSupport24 = {
      source match {
        case support: ReadSupport24 =>
          support
        case _ =>
  ...
  private def tableFromOptions(options: Map[String, String]): Option[TableIdentifier] = {
    options
      .get(TABLE_KEY)
      .map(TableIdentifier(_, options.get(DATABASE_KEY)))
  }
}
